...
const  handleClerkWebhook = httpAction( async (ctx, req) => {
    ...
    switch(event.type){
        case "user.created" :
            const user = await ctx.runQuery(internal.functions.user.get, {clerkId: event.data.id})

            if(user){
                console.log(`Updating use ${event.data.id} with ${event.data}`)
            }
        case "user.updated":
            console.log(`Creating/Updating user: ${event.data.id}`) 

            await ctx.runMutation(
                internal.functions.user.create , {
